...
function populateStates(countryElementId, stateElementId,selected_state) {
    ...
    for (var i = 0; i < state_arr.length; i++) {
		if((selected_state != null || selected_state != "") && (state_arr[i] == selected_state))
		{
			opt = new Option(state_arr[i], state_arr[i]);
			opt.selected = true;
			stateElement.options[stateElement.length] = opt;
		}else{
			opt = new Option(state_arr[i], state_arr[i]);
			stateElement.options[stateElement.length] = opt;
		}
...
function populateCountries(countryElementId, stateElementId, selected_country, selected_state ) {
    ...
    for (var i = 0; i < country_arr.length; i++) {
		if((selected_country != null || selected_country != "") && (country_arr[i] == selected_country))
		{
			opt = new Option(country_arr[i], country_arr[i]);
			opt.selected = true;
    ...
    if (stateElementId) {
		...
		{
			populateStates(countryElementId, stateElementId,selected_state);
		}

    ...
    <div class="row">
        <div class="col-sm-3 col-md-3 col-lg-2">
            ...
            <ul class="nav nav-pills nav-stacked nav-msg">
                ...
                <li><a href="<?=base_url('employee/messageboard/sentbox')?>"><i class="glyphicon glyphicon-send"></i> Sent Message</a></li>
            ...
            <div class="row">        
                <div class="btn-group pull-left ml10 mr10">
                    ...
                    <ul class="dropdown-menu" role="menu">
                      <li>
                          <a href="#" onClick="return unread_selected(document.msglist)">
                           Mark as Unread
                          </a>
                 ...
                 <div class="col-sm-7">
                        ...
                        name="search_msg_inbox"
                        value="<?php if($this->session->userdata('search_msg_inbox')) 
                        echo $this->session->userdata('search_msg_inbox') ?>"
                         class="form-control">
                    </div>
                    <div class="col-sm-3">
                        <input type="submit" name="search_inbox" 
                        class="btn btn-primary btn-metro mr10" value="Search" />
                        <input type="submit" name="all_inbox" class="btn btn-primary btn-metro mr10" value="All" />
                         </form> 
                    </div>
            </div>
                   ...
                   <div class="row">
                        ...
                        <div class="btn-group pull-left mt10">
                            ...
                            <ul class="dropdown-menu pull-left" role="menu">
                                <li>
           <a href="<?=base_url('employee/messageboard/markunread/'.$this->functions->encode($message['InboxId']))?>">Mark as Unread</a>
                                </li>
                                <li>
                                    <a href="<?=base_url('employee/messageboard/reply/inbox/'.$this->functions->encode($message['InboxId']))?>">Reply</a>
                                </li>
                                <li>
                                    <a href="<?=base_url('employee/messageboard/forward/inbox/'.$this->functions->encode($message['InboxId']))?>">Forward</a>
                                </li>
                                <li>
                                    <a href="<?=base_url('employee/messageboard/movetotrash/inbox/'.$this->functions->encode($message['InboxId']))?>">Move to Trash</a>
                                </li>
                            </ul>
                        </div>
                        <div style="cursor:pointer" 
                        onclick="readnew('<?=$message['InboxId']?>',
...
function unread_selected(frm)
{
	if(confirm("Are you sure to Change ?"))
	{
		frm.action.value = "unread_selected";
		frm.submit();
		return true ;
	}
}
...
{
	if($('li#'+id).hasClass('new'))
	{
		$.get( "<?=base_url('employee/messageboard/read_new');?>"+"/"+id, function( data ){
			$('li#'+id).removeClass('new');
			window.location='<?=base_url('employee/messageboard/view/inbox')?>/'+encoded;
		});
	}
	else
	{

...

{
...

    {
		...
		return isset($res['ProjectCategoryName'])?$res['ProjectCategoryName']:'';
...

    {
		...
		return isset($res['ProjectName'])?$res['ProjectName']:'';
	...
	function GetExpenseTypeById($id)
	{
		...
		$query=$this->db->get();
		$res = $query->row_array();
		return isset($res['ExpenseType'])?$res['ExpenseType']:'';	
	} 
	...
	function GetReceivedAmount($invoiceNo=0)
	{
		...
		$query=$this->db->get();
		$res = $query->row_array();
		return issset($res['Amount'])?$res['Amount']:'';
	}
	//==========================================================
	function GetInvoiceAmount($ProjectId=0)
	{
		...
		$query=$this->db->get();
		$res = $query->row_array();
		return isset($res['Amount'])?$res['Amount']:'';
	}
